...
export function* loadFilterTransactionsSaga() {
    ...
    const transactions = yield call(TransactionsStorage.loadFiltered, filters); // 从DB获取符合filter条件的transactions数据
    yield put(loadFilterTransactionsSuccess(transactions)); // 触发load_success action，action的payload就是上一个语句获取的数据库数据
}
...
export function* saveTransactionSaga(action) {
    ...
    console.log('check: '+JSON.stringify(action.payload))// id:''
    const next = action.payload;
    const prev = yield call(TransactionsStorage.remove, next.id);
    next.id = `T${next.date}-${Date.now()}`; // id: "T1679702400000-1679754895732"
